...
namespace GraphFlow::ImageSegmentation::Image {
using namespace cv;
...
GMM::GMM(Mat& _model) {
  ...
  for (int ci = 0; ci < componentsCount; ci++)
    if (coefs[ci] > 0) calcInverseCovAndDeterm(ci, 0.0);
  totalSampleCount = 0;
}
...
double GMM::standardNormal(int ci, const cv::Vec3d color) const {
  ...
  if (coefs[ci] > 0) {
    ...
    diff[1] -= m[1];
    diff[2] -= m[2];
    diff[0] /= covDeterms[ci];
    diff[1] /= covDeterms[ci];
    diff[2] /= covDeterms[ci];
    double mult = diff[0] * diff[0] + diff[1] * diff[1] + diff[2] * diff[2];
