...
FlowGraph::ArcType FlowGraph::arcType(const ListDigraph::Arc& a){
  auto s = digraph.source(a);
  auto t = digraph.target(a);

  if(s==terminalSource) return ArcType::FromSource;
  else if(t==terminalTarget) return ArcType::ToTarget;
  else return ArcType::BetweenNode;
}

void FlowGraph::printEdges(FlowGraph::ArcType at){

  for(ListDigraph::ArcIt a(digraph);a!=lemon::INVALID;++a){
    if(arcType(a)!=at) continue;
    auto ns = digraph.source(a);
    Point ps = ntp[ns];

    std::cout << "(" << ps[0] << "," << ps[1] << ") -> Source:" << arcWeightMap[a] << std::endl;
  }
}

void FlowGraph::printGraph(std::ostream& os){
  os << "Total Nodes (excluding source and target):" << this->ptn.size() << std::endl;

  os << "******Source edges*******" << std::endl;
  printEdges(FromSource);
  os << "\n******Target edges*******" << std::endl;
  printEdges(ToTarget);
  os << "\n******BetweenNode edges*******" << std::endl;
  printEdges(BetweenNode);
