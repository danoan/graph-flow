...
NeighborhoodExplorer<TCombinator,TUserVars,TParams,TContext>::~NeighborhoodExplorer()
{
    ...
    {
        for(auto ep:it->vars.epVector) delete ep.first;
    }
    delete threadTrigger;
}

template<class TCombinator, class TUserVars, class TParams,class TContext>
NeighborhoodExplorer<TCombinator,TUserVars,TParams,TContext>::NeighborhoodExplorer(MyCombinator& combinator, const Context& context):
...
void NeighborhoodExplorer<TCombinator,TUserVars,TParams,TContext>::start(VisitNeighborFunction vnf,int numThreads)
{
    ...
    threadTrigger = new MyThreadTrigger(numThreads,1,[&myContext,&vnf](MyResolver& resolver, MyThreadInput& ti, ThreadControl& tc)
    {
