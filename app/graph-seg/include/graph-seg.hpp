...
DigitalSet graphSeg(const App::GraphSegInput &gfi, TNeighborhood &&neighborhood,
                    std::ostream &os, App::IterationCallback &icb) {
  ...
  double lastEnergyValue =
      App::Graph::evaluateData(id, ds, gfi.dataDistribution) +
      App::Utils::evaluateEnergy(id, ds);
  icb(App::GraphSegIteration(itNumber++, lastEnergyValue, ds,
                             App::GraphSegIteration::Init));
  ...
  while (executing) {
    ...
    auto neighExplorer =
        App::createNeighborExplorer<App::UserVars, App::Params>(range, context);
    ...
    if (id.iterations == -1) {
      ...
      if (id.tolerance >= 0) {
        if (fabs(lastEnergyValue - energyValue) <= id.tolerance){
          executing = false;
        }
      }
    ...
    icb(App::GraphSegIteration(itNumber, lastEnergyValue, ds,
                               App::GraphSegIteration::Running));

    ++itNumber;
  }

  icb(App::GraphSegIteration(itNumber, lastEnergyValue, ds,
                             App::GraphSegIteration::End));
