...
#include <graph-flow/image-segmentation/graph-seg.h>
...
DigitalSet prepareShape(const ImageSegmentation::Image::DataDistribution& DD) {
...
void renderSegmentation(cv::Mat& bcImage, const DigitalSet& ds,
                        const ImageSegmentation::Image::DataDistribution& DD) {
...
void setGraphSegInput(const App::InputData& id,
                      ImageSegmentation::GraphSegInput& gsi) {
...
int main(int argc, char* argv[]) {
  ...
  ImageSegmentation::Image::DataDistribution DD(gco, id.grabcutIterations);
  ...
  ImageSegmentation::IterationCallback iterationCallback =
    ...
    switch (gfIteration.iterationState) {
      ...
      case ImageSegmentation::GraphSegIteration::Running: {
        if (id.saveAllFigures) {
          Utils::Display::saveDigitalSetAsImage(
              ds, id.outputFolder + "/" +
                      Utils::String::nDigitsString(gfIteration.iteration, 4) + ".png");
        }
      ...
      case ImageSegmentation::GraphSegIteration::End: {
  ...
  switch (id.neighborhoodType) {
    case App::InputData::Morphology: {
      Core::Neighborhood::Morphology M(
          ...
          id.neighborhoodSize);
      outputDS = ImageSegmentation::graphSeg(gsi, M, ofsEnergy, iterationCallback);
      break;
    }
    case App::InputData::Random: {
      Core::Neighborhood::Random R(id.neighborhoodSize);
      outputDS = ImageSegmentation::graphSeg(gsi, R, ofsEnergy, iterationCallback);
      break;
    }
  }
