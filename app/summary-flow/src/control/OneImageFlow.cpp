...
OneImageFlow::Domain OneImageFlow::findCommonDomain(
...
void OneImageFlow::drawContours(DrawingBuffer& drawBuffer,
  ...
  auto pointDraw = [&drawBuffer, &itColor](const Point& p) {
    drawBuffer.draw(p, *itColor, *itColor);
  };

  // Draw intermediate contours
  while (itContour != contours.end()) {
    std::for_each(itContour->begin(), itContour->end(), pointDraw);
  ...
  auto pointDrawFirst = [&drawBuffer, &colorScheme](const Point& p) {
    drawBuffer.draw(p, colorScheme.first(), colorScheme.first());
  };
  itContour = contours.begin();
  std::for_each(itContour->begin(), itContour->end(), pointDrawFirst);

  // Draw last contour
  auto pointDrawLast = [&drawBuffer, &colorScheme](const Point& p) {
    drawBuffer.draw(p, colorScheme.last(), colorScheme.last());
  };
  while (itContour != contours.end()) ++itContour;
  --itContour;
  std::for_each(itContour->begin(), itContour->end(), pointDrawLast);
}
...
void OneImageFlow::createUnifiedImage(const std::string& imgOutputPath,
                                      ...
                                      const ColorScheme& colorScheme,
                                      int drawInterval, ImageOutputType iot) {
  Domain domain = findCommonDomain(contours);
  DrawingBuffer drawBuffer;
  drawContours(drawBuffer, contours, drawInterval, colorScheme);

  // Draw ExtraPoints
  auto pointDrawExtra = [&drawBuffer, &colorScheme](const Point& p) {
    drawBuffer.draw(p, colorScheme.extra(), colorScheme.extra());
  };
  std::for_each(extraPoints.begin(), extraPoints.end(), pointDrawExtra);

  if (iot == SVG)
    drawBuffer.flushSVG((imgOutputPath).c_str());
  else
    drawBuffer.flushEPS((imgOutputPath).c_str());
}
...
void OneImageFlow::collectImagesPath(
    std::vector<std::string>& paths, const std::string& imageSrcFolder,
  ...
  while (di != boost::filesystem::directory_iterator()) {
    ...
    if (boost::filesystem::is_regular_file(currPath) &&
        currPath.extension().string() == imageExtension) {
...
OneImageFlow::OneImageFlow(const std::string& imageSrcFolder,
  ...
  collectImagesPath(vectorOfImgPath, imageSrcFolder, imageExtension,
                    [&pixelMaskPath, &dirsMaskPath](const std::string& s) {
                      return s != pixelMaskPath && s != dirsMaskPath;
                    });
  ...
  createUnifiedImage(imageOutputPath, contours, extraPoints, colorScheme,
                     drawInterval, iot);
}
}  // namespace SummaryFlow
