#ifndef GRAPH_FLOW_SEED_SELECTOR_SEEDPICKER_MASKSELECTOR_H
#define GRAPH_FLOW_SEED_SELECTOR_SEEDPICKER_MASKSELECTOR_H
...
class MouseEvent : public SeedSelector::IMouseEvent {
 public:
  MouseEvent(const cv::Mat& cvImg, cv::Scalar rectColor = cv::Scalar(0, 255, 0),
  ...
  static void callbackFn(int event, int x, int y, int flags, void* param);

  const cv::Mat& blendImage() {
    std::cout << _rectColor << std::endl;
    return this->_blendImage;
  }
...
#endif  // GRAPH_FLOW_SEED_SELECTOR_SEEDPICKER_MASKSELECTOR_H
