...
void displayStatusBar(const GUIData& gd) {
  std::string penWidthText =
      "Pen Width: " + std::to_string((unsigned int)gd.penWidth);
...
void setRectMouseCallback(GUIData& gd) {
  ...
  cv::setMouseCallback(gd.windowName,
                       SeedSelector::RectSelector::MouseEvent::callbackFn,
                       gd.me());
}
...
void setMaskMouseCallback(GUIData& gd) {
  cv::setMouseCallback(gd.windowName, NULL);
  cv::setMouseCallback(gd.windowName,
                       SeedSelector::MaskSelector::MouseEvent::callbackFn,
                       gd.me());
}
...
void refreshWindow(GUIData& gd) {
  ...
  hasChanged = hasChanged || gd.me()->hasChange();
  gd.me()->change(false);
  ...
  if (hasChanged) cv::imshow(gd.windowName, blendImg);
}
...
void probablyForegroundCallback(int state, void* userdata) {
  ...
  cv::displayOverlay(gd->windowName, "Select probable foreground region.",
                     1000);
}
...
void restartCallback(int state, void* userdata) {
  GUIData* gd = (GUIData*)userdata;
  gd->me()->restart();
  cv::displayOverlay(gd->windowName, "Current region has been restarted!",
                     1000);
}
...
void saveCallback(int state, void* userdata) {
  ...
  std::string bgMaskOutput =
      gd->outputFolder + "/mask-bg-" + std::to_string(gd->seedCounter) + ".pgm";
  std::string fgMaskOutput =
      gd->outputFolder + "/mask-fg-" + std::to_string(gd->seedCounter) + ".pgm";
  std::string pbfgMaskOutput = gd->outputFolder + "/mask-pbfg-" +
                               std::to_string(gd->seedCounter) + ".pgm";
